...
def make_pairs(x, y):
    # 构造paris匹配和不匹配的示例
    # 并构造标签（匹配为1，不匹配为0）
    num_classes = int(max(y) + 1)  # 输出为：3类
    digit_indices = [np.where(y == i)[0] for i in range(num_classes)]  # 将属于每个类的索引保存在此
    ...
    for idx1 in range(len(x)):
        ...
        true_labels += [label1, label1]
    # print("pairs:",pairs)
    # print("labels:", labels)
    # print("source_labels:", source_labels)

    return np.array(pairs), np.array(labels).astype("float32"), np.array(source_labels).astype("float32"), true_labels
...
def load_images(basedir, path, input_size):
    batches = ImageDataGenerator(rescale=1 / 255.).flow_from_directory(os.path.join(basedir, path),
                                                                       ...
                                                                       shuffle=True,
                                                                       seed=42)
    print(batches.filenames)
    print(batches.filepaths)
    # print(batches.class_indices)
    return batches[0][0], batches[0][1], batches.filenames


def visualize(pairs, labels, to_show=6, num_col=3, predictions=None, test=False):
    """Creates a plot of pairs and labels, and prediction if it's validation dataset.
